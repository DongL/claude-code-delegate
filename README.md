...
DeepSeek V4 Pro is not available on OpenCode Zen. OpenCode Zen currently has only 1 DeepSeek model: `deepseek-v4-flash-free` (free beta, $0/1M tokens) — `deepseek-v4-flash` is served as an alias.
...
- `--flash` → `deepseek/deepseek-v4-flash`
- `--qwen` → `opencode/qwen3.6-plus-free`
DeepSeek V4 Pro 在 OpenCode Zen 上不可用。OpenCode Zen 目前仅有 1 个 DeepSeek 模型：`deepseek-v4-flash-free`（免费 beta，$0/1M tokens）——`deepseek-v4-flash` 作为别名提供服务。
